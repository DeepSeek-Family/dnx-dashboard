...
} from '@/store/api/dashboardOverViewPage/dashboardOverview'
...
export default function DashboardPage() {
  ...
  const { data: monthlyProgress, isLoading: isMonthlyProgressLoading } =
    useGetMOnthlyProgressQuery({})
  ...
  return (
    <div className="space-y-8 md:space-y-10">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-dnx-muted">
        ...
        <Col xs={24} sm={12} xl={6}>
          {kpiLoading ? (
            ...
            <MetricCard
              label="Subscriptions"
              value={
                <AnimatedNumber value={dashboardOverviewData.totalSubscription ?? 0} />
              }
        ...
        <Col xs={24} sm={12} xl={6}>
          {kpiLoading ? (
            ...
            <MetricCard
              label="Live Sessions"
              value={
                <AnimatedNumber value={dashboardOverviewData.totalLiveSessions ?? 0} />
              }
          ...
          <div className="h-[260px] w-full">
            <ResponsiveContainer>
              <LineChart data={chartData}>
                ...
                <YAxis stroke="#8B91A8" />
                <Tooltip
                  contentStyle={{ background: '#131726', borderColor: '#2B3048' }}
                />
                <Line
                  type="monotone"
                  dataKey="users"
      ...
      <Row gutter={[20, 20]} className="mt-12">
        <Col span={24}>
          <Card
            title={
              <span className="text-sm font-medium text-white">Pending Requests</span>
            }
            className="glass-card !border-dnx-border/80 !bg-dnx-card/80 !shadow-none"
            bodyStyle={{ padding: 20 }}
          >
            <div className="space-y-4 text-sm">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-dnx-muted">
                    4 gyms waiting for admin review
                  </p>
                </div>
                <Button
                  size="small"
                  className="rounded-full border-dnx-yellow/60 text-dnx-yellow"
                >
